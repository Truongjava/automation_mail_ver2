...
# app = FastAPI(title="Email API: Local File Attachments + Inline Images")

# # Load cấu hình SMTP
# with open("smtp_config.json", "r") as f:
#     SMTP_CONFIGS = json.load(f)

# # Đọc file cục bộ (ảnh và PDF)
...
#         try:
#             # === Đọc ảnh và PDF từ thư mục hiện tại ===
#             image1 = load_local_file("banner.jpg")
#             image2 = load_local_file("logo.jpg")
#             pdf_file = load_local_file("Pitch Deck.pdf")
...
#             # Chuyển \n trong body thành <br> để HTML hiển thị đúng dòng
#             body_html = body.replace("\n", "<br>")
...
app = FastAPI(title="Email API: Dynamic SMTP + Inline Images + Attachments")
...
async def send_email_dynamic(
    ...
    smtp_user: str = Form(...),
    smtp_pass: str = Form(...),
    smtp_server: str = Form(default="smtp.gmail.com"),
    ...
    result = {
        ...
        "subject": subject,
        "body": body,
        "assigned_account_email": smtp_user,
        "sent_time": datetime.now(ZoneInfo("Asia/Ho_Chi_Minh")).isoformat(),
    }
